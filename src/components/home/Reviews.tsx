...
const reviews = [
  {
    ...
    text: 'Driver was waiting when we landed, name board, helped with all luggage. Price was exactly as quoted — no extras. Comfortable Mercedes, smooth ride. Will absolutely use again.',
    stars: 5,
  },
  {
    name: 'Sophie M.',
  ...
  },
  {
    ...
    text: 'The coastal road is not for the faint-hearted but our driver handled it perfectly. Knew every bend, arrived safely. Professional service from start to finish.',
    stars: 5,
  },
  {
    name: 'Claire D.',
  ...
  },
  {
    ...
    text: 'Booked two days before via WhatsApp — replied within minutes. Driver at the port exactly on time. Took all 6 bags without any issue. Saved us enormous stress.',
    stars: 5,
  },
  {
    name: 'Lena V.',
    country: '🇨🇭 Switzerland',
    route: 'Milan → Lugano',
    text: 'Cross-border transfer booked for my parents. They don\'t speak Italian well — but the driver was patient and kind, handled the border with no fuss. Truly professional.',
    stars: 5,
  },
]
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-14">
          <div>
            <div className="flex items-center gap-4 mb-5">
              ...
              <span className="section-label">Verified Passenger Reviews</span>
            </div>
            <h2 className="font-black leading-[0.95]" style={{ fontSize: 'clamp(2.2rem, 4vw, 3.5rem)', color: '#080808' }}>
          ...
          <div className="lg:text-right">
            ...
            <p className="text-sm text-gray-500">Based on 847+ verified passenger reviews</p>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
          {reviews.map((r, i) => (
            <div
              key={i}
              className="bg-white rounded-sm p-7 flex flex-col"
            ...
            >
              ...
              <div className="flex items-center gap-3 pt-4" style={{ borderTop: '1px solid rgba(201,168,76,0.1)' }}>
                ...
                <div>
                  <p className="font-bold text-gray-900 text-sm leading-none mb-0.5">{r.name}</p>
        ...
        <div className="text-center">
          <Link
            ...
            className="inline-flex items-center gap-4 rounded-sm px-8 py-5 transition-all hover:-translate-y-0.5 hover:shadow-lg"
            style={{ background: '#080808', border: '1px solid rgba(201,168,76,0.2)' }}
          >
            <div className="flex gap-0.5">
              {[...Array(5)].map((_, i) => (
                <svg key={i} className="w-4 h-4" viewBox="0 0 24 24" fill="#C9A84C"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
              ))}
            </div>
