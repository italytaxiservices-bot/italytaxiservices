import Link from 'next/link'

const WHATSAPP = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER ?? '393148932631'

export default function FinalCTA() {
  const waUrl = `https://wa.me/${WHATSAPP}?text=Hello%2C%20I%20would%20like%20to%20book%20a%20private%20transfer%20in%20Italy.%20Can%20you%20send%20me%20a%20quote%3F`

  return (
    <section className="relative overflow-hidden grain" style={{ background: '#050505' }}>
      <div className="absolute inset-0">
        <div
          className="absolute inset-0"
          style={{
            backgroundImage: 'url("https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?auto=format&fit=crop&w=1920&q=80")',
            backgroundSize: 'cover',
            backgroundPosition: 'center',
            opacity: 0.1,
          }}
        />
      </div>

      <div className="h-[1px]" style={{ background: 'linear-gradient(90deg, transparent 5%, rgba(201,168,76,0.4) 40%, rgba(224,192,112,0.6) 60%, rgba(201,168,76,0.4) 80%, transparent 95%)' }} />

      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 py-32">
        <div className="grid lg:grid-cols-[1fr_340px] gap-20 items-center">

          {/* Copy */}
          <div>
            <div className="flex items-center gap-4 mb-8">
              <div className="gold-line" />
              <span className="section-label">Ready When You Are</span>
            </div>

            <h2 className="text-white font-black leading-[0.95] mb-6" style={{ fontSize: 'clamp(2rem, 4.5vw, 4rem)' }}>
              Your Chauffeur.<br />
              <span className="text-gold-gradient italic font-bold" style={{ fontFamily: 'var(--font-serif), Georgia, serif' }}>
                Your Schedule.
              </span>
              <br />
              Fixed Price.
            </h2>

            <p className="text-base leading-relaxed mb-10 max-w-md" style={{ color: 'rgba(255,255,255,0.6)' }}>
              From Malpensa to Lake Como. From Fiumicino to Rome. From Naples to the Amalfi Coast.
              Every transfer confirmed at a fixed price — no surprises, no meters, no stress.
            </p>

            {/* Mini trust strip */}
            <div className="flex flex-wrap gap-6">
              {[
                { label: 'Licensed NCC', icon: '✓' },
                { label: 'Fixed Price', icon: '✓' },
                { label: '24 / 7 Service', icon: '✓' },
                { label: 'Free Cancellation', icon: '✓' },
              ].map(({ label, icon }) => (
                <div key={label} className="flex items-center gap-2">
                  <span className="text-xs font-bold" style={{ color: '#C9A84C' }}>{icon}</span>
                  <span className="text-xs" style={{ color: 'rgba(255,255,255,0.55)' }}>{label}</span>
                </div>
              ))}
            </div>
          </div>

          {/* Actions */}
          <div className="flex flex-col gap-4">
            <Link href="/#quote-form" className="btn-primary justify-center text-center rounded-sm py-5 text-sm">
              Get a Free Quote
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
            </Link>

            <a
              href={waUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2.5 py-4 rounded-sm font-semibold text-sm transition-all hover:-translate-y-0.5"
              style={{ background: 'rgba(37,211,102,0.1)', border: '1px solid rgba(37,211,102,0.3)', color: '#4ade80' }}
            >
              <svg className="w-4 h-4" fill="#25D366" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
              WhatsApp Us Directly
            </a>

            <div className="grid grid-cols-3 gap-3 pt-1">
              {['No booking fee', '< 2hr reply', 'Free cancel'].map((t) => (
                <div key={t} className="text-center py-3 rounded-sm" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(201,168,76,0.12)' }}>
                  <p className="text-[10px] uppercase tracking-widest" style={{ color: 'rgba(255,255,255,0.65)' }}>{t}</p>
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>
    </section>
  )
}
